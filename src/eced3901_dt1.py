...
class SquareMoveOdom(SquareMove):
	...
	# changed 0.5 to 0.1 so the robot moves in 0.5 increments.
    ...
    def turn_of(self, a, ang_speed=0.4):
	    ...
	    # while has been set up with absolute values
            ...
            # sys.stdout.flush()
            # print abs(self.get_z_rotation(self.odom_pose.orientation) - a_init)
    ...
    def move(self):
        ...
        self.move_of(0.5)
        self.turn_of(math.pi/2.2)
        self.move_of(0.48)
        self.turn_of(math.pi/2.4)
        self.move_of(0.565)
        self.turn_of(math.pi/2.39)
        self.move_of(0.588)

	# I want it to go back to its original orientiation for easier repeated simulation.
	self.turn_of(math.pi/2.1)
        self.stop_robot()
